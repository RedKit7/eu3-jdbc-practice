...
public class jsonToJavaCollection {
    ...
    public void regionToMap(){
        ...
        Response response = when().get("http://54.165.32.99:1000/ords/hr/regions");

        assertEquals(response.statusCode(),200);

        //we de-serialize JSON response to Map
        Map<String,Object> regionMap = response.body().as(Map.class); // i use restassured as method to de-serialize (and also i should add the librarriees: gson or jackson)
